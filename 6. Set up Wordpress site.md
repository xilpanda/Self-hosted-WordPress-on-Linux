...
Replace all instances of 'sandrolinux' with the system username that you'll use for this site. It makes sense to use a truncated version of your domain for this, e.g. for 'sandrolinux.com' I would use 'sandrolinux'.


## Create a system user for this site

I'm using the username 'sandrolinux' in all of these examples, but you can change it to something that reflects your website's name

    export WEBSITEUSER=sandrolinux
...
Add the following content to /etc/nginx/conf.d/sandrolinux.conf. Replace all occurrences of 'sandrolinux' in this file with your IP address or domain name (if you have one):

```bash
nano /etc/nginx/conf.d/sandrolinux.conf
```

```
server {
    listen       80;
    server_name  www.sandrolinux.com;

    client_max_body_size 20m;

    index index.php index.html index.htm;
    root   /home/sandrolinux/public_html;
    ...
    location ~ \.php$ {
            ...
            fastcgi_pass unix:/var/run/php/sandrolinux.sock;
...
server {
    listen       80;
    server_name  sandrolinux.com;
    rewrite ^/(.*)$ http://www.sandrolinux.com/$1 permanent;
}
```
...
```bash
nano /etc/php/8.3/fpm/pool.d/sandrolinux.conf
```

Replace all occurrences of "sandrolinux" in the configuration file content below with your site name.

```
[sandrolinux]
listen = /var/run/php/sandrolinux.sock
listen.owner = sandrolinux
...
php_admin_value[error_log] = /home/sandrolinux/logs/phpfpm_error.log
php_admin_value[open_basedir] = /home/sandrolinux:/tmp
```
...
```bash
sudo -u sandrolinux touch /home/sandrolinux/logs/phpfpm_error.log
```
...
Replace `chooseapassword` with the strong password that you just created with the shell command above, and `sandrolinux` with your site name.

```
# Log into mysql
CREATE DATABASE sandrolinux;
CREATE USER 'sandrolinux@localhost' IDENTIFIED BY 'chooseapassword';
GRANT ALL PRIVILEGES ON sandrolinux.* TO sandrolinux@localhost;
FLUSH PRIVILEGES;
```
...
Become your site user (named sandrolinux in my case) and download the WordPress application:
...
Make sure you're in your user's home/public_html directory. I'm still using the WEBSITEUSER variable that we set earlier -- in my case it's set to `sandrolinux`:
...
```
1.2.3.4    sandrolinux.com
1.2.3.4    www.sandrolinux.com
```
...
```bash
chmod 640 /home/sandrolinux/public_html/wp-config.php
```
